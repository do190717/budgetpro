...
export default function FinanceTab({ state, onStateChange }: FinanceTabProps) {
  ...
  const handleToggleDeduction = (deductionId: string) => {
    const updated = state.deductions.map(d => d.id === deductionId ? { ...d, enabled: !d.enabled } : d);
  ...
  const handleAddPayment = () => {
    ...
    const payment: MaasarPayment = { id: generateId(), date: paymentDate, desc: paymentDesc.trim(), amount: parseFloat(paymentAmount) };
    onStateChange(addMaasarPayment(state, payment));
    setPaymentDesc(''); setPaymentAmount('');
    setPaymentDate(new Date().toISOString().split('T')[0]);
    setIsAddingPayment(false); setAddMode(null);
  };

  const handleDeletePayment = (paymentId: string) => {
    setConfirmState({ open: true, title: 'האם למחוק את ההפרשה?', onConfirm: () => onStateChange(deleteMaasarPayment(state, paymentId)) });
  };
  ...
  const updateDeductionName = (deductionId: string, name: string) => {
    const updated = state.deductions.map(d => d.id === deductionId ? { ...d, name } : d);
    onStateChange(updateDeductions(state, updated));
  };
  ...
  const handleAddRecurring = () => {
    ...
    const rp: RecurringPayment = { id: generateId(), desc: recurringDesc.trim(), amount: parseFloat(recurringAmount), dayOfMonth: day, lastRegistered: '', enabled: true };
  ...
  const handleDeleteRecurring = (id: string) => {
    setConfirmState({ open: true, title: 'האם למחוק את ההפרשה הקבועה?', onConfirm: () => onStateChange(deleteRecurringPayment(state, id)) });
  };
      ...
      <section>
        ...
        <div className="border border-gray-300 rounded-xl overflow-hidden bg-white">
          <button onClick={() => setIsDeductionsOpen(!isDeductionsOpen)}
            className="w-full px-4 py-3 bg-gray-100 border-b border-gray-300 flex justify-between items-center cursor-pointer transition-colors">
            <div className="flex items-center gap-2">
              <span className="text-gray-500" style={{ display: 'inline-block', transform: isDeductionsOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>▶</span>
              <h2 className="font-semibold text-gray-700">ניכויים</h2>
            </div>
            <span onClick={(e) => { e.stopPropagation(); addNewDeduction(); setIsDeductionsOpen(true); }} className="text-gray-600 hover:text-gray-800 text-sm font-medium">
              + הוסף ניכוי
            </span>
          </button>
              ...
              {state.deductions.map(deduction => (
                <div key={deduction.id} className="flex items-center transition-colors"
                  ...
                  <div className="flex-shrink-0 px-2 sm:px-3">
                    <input type="checkbox" checked={deduction.enabled} onChange={() => handleToggleDeduction(deduction.id)} className="w-4 h-4 sm:w-5 sm:h-5 rounded border-gray-400 cursor-pointer" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <input type="text" value={deduction.name} onChange={e => updateDeductionName(deduction.id, e.target.value)}
                  ...
                  <div className="flex-shrink-0 flex items-center">
                    <input type="number" value={deduction.pct} onChange={e => handleUpdateDeductionPct(deduction.id, parseFloat(e.target.value) || 0)}
                      className="text-gray-700 text-center" style={{ width: '45px', border: 'none', background: 'transparent', outline: 'none', fontSize: '13px', padding: '7px 4px' }} min="0" max="100" />
                    <span className="text-gray-500 text-sm">%</span>
                  </div>
                  <div className={`flex-shrink-0 text-left font-medium ${deduction.enabled ? 'text-gray-700' : 'text-gray-400'}`} style={{ fontSize: '13px', padding: '7px 8px', minWidth: '70px' }}>
                    {formatCurrency(calcDeductionAmount(totalProfit, deduction))}
                  </div>
                  <button onClick={() => deleteDeduction(deduction.id)} className="flex-shrink-0 text-gray-400 hover:text-red-500 px-2 sm:px-3 py-2">✕</button>
                </div>
              ))}
            </div>
          ...
          <div className="px-4 py-4 flex items-center justify-between border-b border-amber-200" style={{ backgroundColor: '#FEF9E7' }}>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1">
                <input type="number" value={state.maasarPct} onChange={e => handleUpdateMaasarPct(parseFloat(e.target.value) || 0)}
                  className="w-14 px-2 py-1 border border-amber-300 rounded text-center bg-white text-amber-800 font-medium" min="0" max="100" />
        ...
        <div className="border border-gray-300 rounded-xl overflow-hidden bg-white">
          <div className="px-4 py-3 bg-gray-100 border-b border-gray-300 flex justify-between items-center">
            ...
            {!addMode && !isAddingPayment && (
              <button onClick={() => setAddMode('choose')} className="bg-green-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-green-700">+ הפרשה חדשה</button>
            )}
          </div>
            ...
            <div className="p-4 bg-green-50 border-b border-green-200">
              <div className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">תאריך</label>
                    <input type="date" value={paymentDate} onChange={e => setPaymentDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">תיאור</label>
                    <input type="text" value={paymentDesc} onChange={e => setPaymentDesc(e.target.value)} placeholder="לדוגמה: תרומה לעמותה" className="w-full px-3 py-2 border border-gray-300 rounded-lg" autoFocus /></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">סכום (₪)</label>
                    <input type="number" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} placeholder="0" className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
                </div>
                <div className="flex gap-2">
                  <button onClick={handleAddPayment} className="px-4 py-2 rounded-lg bg-green-600 text-white font-medium">הוסף</button>
                  <button onClick={() => { setIsAddingPayment(false); setPaymentDesc(''); setPaymentAmount(''); setAddMode(null); }} className="px-4 py-2 rounded-lg bg-gray-200 text-gray-700">ביטול</button>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="p-4 border-b" style={{ background: '#EFF6FF' }}>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div><label className="block text-xs text-gray-500 mb-1 text-right">סכום ₪</label>
                  <input type="number" value={recurringAmount} onChange={e => setRecurringAmount(e.target.value)} placeholder="0" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" autoFocus /></div>
                <div><label className="block text-xs text-gray-500 mb-1 text-right">יום בחודש (1-28)</label>
                  <input type="number" value={recurringDay} onChange={e => setRecurringDay(e.target.value)} min="1" max="28" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
              </div>
                  ...
                  {state.maasarPayments.map(payment => (
                    <tr key={payment.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <button onClick={() => handleDeletePayment(payment.id)} className="text-gray-400 hover:text-red-500">✕</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
